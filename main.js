	...
	CookieAssistant.launch = function()
	{
		...
		CookieAssistant.i18n = {
			"JA": {
				...
				// menu texts
				menu_text_0: '※この機能はアップグレードの生贄用めん棒も自動で購入するようになります。',
				menu_text_1: "完了後の切り替え先",
				menu_text_2: "アップグレードが残っているシーズンに自動的に切り替えます。",
				menu_text_3: "⚠️パンテオンがロックされています。アンロックするまでこの機能は使えません。",
				menu_text_4: "発動後にやること",
				menu_text_5: "⚠️ゴジャモックがセットされていないため、有効化しても恩恵が無い可能性があります。",
			...
			"EN": {
				...
				goldenSwitch_enable_3: "Have two or more buffs including click buff",
				...
				notify_1: "You have not unlocked the Grimoire yet, so failed to spellcast.",
				...
				// menu texts
				menu_text_0: 'This feature will also automatically purchase "Sacrificial rolling pins".',
				menu_text_1: "Switch to after complete",
				menu_text_2: "Automatically switch to seasons in which the upgrade is still remained.",
				menu_text_3: "⚠️You have not unlocked the Pantheon yet. This feature will not be available until it is unlocked.",
				menu_text_4: "Do After Activated",
				menu_text_5: "⚠️Godzamok is not set, so there may be no benefit from enabling this.",
		...
		{
			CookieAssistant.i18n[CookieAssistant.localeId] = CookieAssistant.i18n["EN"];
		}
		...
		CookieAssistant.sellBuildings = function(index, target, amount, activate_mode, after_mode)
		{
			...
			if (CookieAssistant.config.particular.sell.isAfterSell[index])
			{
				...
				if (after_mode == 1)//Spellcast and buy back
				{
					...
					{
						Game.Notify(CookieAssistant.name, CookieAssistant.texts.notify_1, "", 3);
						CookieAssistant.config.particular.sell.isAfterSell[index] = 0;
						return false;
					}
					var spell = grimoire.spells['hand of fate'];
				...
				{
					Game.Notify(CookieAssistant.name, CookieAssistant.texts.notify_2);
				}
				CookieAssistant.config.particular.sell.isAfterSell[index] = 0;
				return false;
			}
			...
			if (isMode0 || isMode1 || isMode2 || isMode3 || isMode4 || isMode5 || isMode6)
			{
				...
				{
					Game.Notify(CookieAssistant.name, CookieAssistant.texts.notify_3);
					return false;
				}
		...
		CookieAssistant.getMenuString = function()
		{
			...
			str +=	'<div class="listing">'
					+ m.ToggleButton(CookieAssistant.config.flags, 'autoClickBigCookie', 'CookieAssistant_autoClickBigCookieButton', `${CookieAssistant.texts.autoClick + " " + CookieAssistant.texts.bigCookie} ON`, `${CookieAssistant.texts.autoClick + " " + CookieAssistant.texts.bigCookie} OFF`, "CookieAssistant.Toggle")
					+ intervalLabel
					+ m.InputBox("CookieAssistant_Interval_autoClickBigCookie", 40, CookieAssistant.config.intervals.autoClickBigCookie, "CookieAssistant.ChangeInterval('autoClickBigCookie', this.value)")
						+ '<label></label><a class="option" ' + Game.clickStr + '=" CookieAssistant.config.particular.bigCookie.isMute++; if(CookieAssistant.config.particular.bigCookie.isMute >= 2){CookieAssistant.config.particular.bigCookie.isMute = 0;} Game.UpdateMenu(); PlaySound(\'snd/tick.mp3\');">'
								+ (CookieAssistant.config.particular.bigCookie.isMute ? CookieAssistant.texts.menu_autoClickBigCookie_mute_SE : CookieAssistant.texts.menu_autoClickBigCookie_play_SE)
						+ '</a><br />'
					+ '<div class="listing">'
						+ modeLabel
					...
					+ '</div>';
			//黄金クッキークリック
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoClickGoldenCookie', 'CookieAssistant_autoClickGoldenCookieButton', `${CookieAssistant.texts.autoClick} ` + loc("Golden cookie") + ' ON', `${CookieAssistant.texts.autoClick} ` + loc("Golden cookie") + ' OFF', "CookieAssistant.Toggle")
					+ intervalLabel
					...
					+ '</div>';
			//虫撃破
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoClickWrinklers', 'CookieAssistant_autoClickWrinklers', `${CookieAssistant.texts.autoClick} ` + loc("wrinkler") + ' ON', `${CookieAssistant.texts.autoClick} ` + loc("wrinkler") + ' OFF', "CookieAssistant.Toggle")
					+ intervalLabel
					...
					+ '</div>';
			//トナカイクリック
			str +=  '<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoClickReindeer', 'CookieAssistant_autoClickReindeerButton', `${CookieAssistant.texts.autoClick} ` + loc("Reindeer") + ' ON', `${CookieAssistant.texts.autoClick} ` + loc("Reindeer") + ' OFF', "CookieAssistant.Toggle");
			...
			str +=	'</div>';
			//FortuneNewsクリック
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoClickFortuneNews', 'CookieAssistant_autoClickFortuneNewsButton', `${CookieAssistant.texts.autoClick + " " + CookieAssistant.texts.fortuneNews} ON`, `${CookieAssistant.texts.autoClick + " " + CookieAssistant.texts.fortuneNews} OFF`, "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'</div>';
			//ElderPedge自動購入
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoBuyElderPledge', 'CookieAssistant_autoBuyElderPledge', `${CookieAssistant.texts.autoBuy} ` + loc("[Upgrade name 74]Elder Pledge") + ' ON', `${CookieAssistant.texts.autoBuy} ` + loc("[Upgrade name 74]Elder Pledge") + ' OFF', "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'<div class="listing">'
					...
					+ '</div>';
			//アップグレード自動購入
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoBuyUpgrades', 'CookieAssistant_autoBuyUpgrades', `${CookieAssistant.texts.autoBuy} ` + loc("upgrade") + ' ON', `${CookieAssistant.texts.autoBuy} ` + loc("upgrade") + ' OFF', "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'<div class="listing">'
					...
					+ '</div>';
			//建物自動購入
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoBuyBuildings', 'CookieAssistant_autoBuyBuildings', `${CookieAssistant.texts.autoBuy} ` + loc("building") + ' ON', `${CookieAssistant.texts.autoBuy} ` + loc("building") + ' OFF', "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'<div class="listing">'
					...
					+ '</div>';
			//砂糖玉自動収穫
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoHarvestSugarlump', 'CookieAssistant_autoHarvestSugarlump', `${CookieAssistant.texts.autoHarvest} ` + loc("sugar lump") + ' ON', `${CookieAssistant.texts.autoHarvest} ` + loc("sugar lump") + ' OFF', "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			//自動詠唱
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoSpellonBuff', 'CookieAssistant_autoSpellonBuff', `${CookieAssistant.texts.autoSpellCast} ` + loc("Force the Hand of Fate") + ' ON', `${CookieAssistant.texts.autoSpellCast} ` + loc("Force the Hand of Fate") + ' OFF', "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			//シーズン自動切換え
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoSwitchSeason', 'CookieAssistant_autoSwitchSeason', `${CookieAssistant.texts.autoSwitchSeasons} ON`, `${CookieAssistant.texts.autoSwitchSeasons} OFF`, "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			//ドラゴン自動育成
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoTrainDragon', 'CookieAssistant_autoTrainDragon', `${CookieAssistant.texts.autoTrainDragon} ON`, `${CookieAssistant.texts.autoTrainDragon} OFF`, "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'<div class="listing">'
						+ `<label>${CookieAssistant.texts.aura} 1 : </label>`
							+ `<select class="option" style="background-color: black; color: white; border-color: #ece2b6 #875526 #733726 #dfbc9a; border: 1px solid; outline: none;" onchange="CookieAssistant.config.particular.dragon.aura1 = parseInt(this.value); Game.UpdateMenu();">`
								+ Object.keys(Game.dragonAuras).map((id) => 
									`<option value="${id}" ${id == CookieAssistant.config.particular.dragon.aura1 ? 'selected' : ''}>${Game.dragonAuras[id].dname}</option>`
								).join('')
							+ '</select>'
						+ `<label>${CookieAssistant.texts.aura} 2 : </label>`
							+ `<select class="option" style="background-color: black; color: white; border-color: #ece2b6 #875526 #733726 #dfbc9a; border: 1px solid; outline: none;" onchange="CookieAssistant.config.particular.dragon.aura2 = parseInt(this.value); Game.UpdateMenu();">`
								+ Object.keys(Game.dragonAuras).map((id) => 
									`<option value="${id}" ${id == CookieAssistant.config.particular.dragon.aura2 ? 'selected' : ''}>${Game.dragonAuras[id].dname}</option>`
								).join('')
							+ '</select><br />'
					+ '</div>'
					+ '</div>';
			...
			//パンテオンのスロット自動セット
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoSetSpirits', 'CookieAssistant_autoSetSpirits', `${CookieAssistant.texts.autoSetSpirits} ON`, `${CookieAssistant.texts.autoSetSpirits} OFF`, "CookieAssistant.Toggle");
					if (CookieAssistant.showAllIntervals)
					{
						str += intervalLabel
			...
			str +=	'<div class="listing">';
					...
					if (Game.Objects['Temple'].minigame != undefined && Game.Objects['Temple'].minigameLoaded)
					{
						str +=	`<label style="color: white;">${CookieAssistant.texts.diamond} : </label>`
								+ `<select class="option" style="background-color: black; color: white; border-color: #ece2b6 #875526 #733726 #dfbc9a; border: 1px solid;" onchange="CookieAssistant.config.particular.spirits.slot1 = parseInt(this.value); Game.UpdateMenu();">`
									+ Object.keys(Game.Objects['Temple'].minigame.gods).map((id, index) => 
										`<option value="${index}" ${index == CookieAssistant.config.particular.spirits.slot1 ? 'selected' : ''}>${Game.Objects['Temple'].minigame.godsById[index].name}</option>`
									).join('')
								+ '</select>'
								...
								+ `<select class="option" style="background-color: black; color: white; border-color: #ece2b6 #875526 #733726 #dfbc9a; border: 1px solid;" onchange="CookieAssistant.config.particular.spirits.slot2 = parseInt(this.value); Game.UpdateMenu();">`
									+ Object.keys(Game.Objects['Temple'].minigame.gods).map((id, index) => 
										`<option value="${index}" ${index == CookieAssistant.config.particular.spirits.slot2 ? 'selected' : ''}>${Game.Objects['Temple'].minigame.godsById[index].name}</option>`
									).join('')
								+ '</select>'
								...
								+ `<select class="option" style="background-color: black; color: white; border-color: #ece2b6 #875526 #733726 #dfbc9a; border: 1px solid;" onchange="CookieAssistant.config.particular.spirits.slot3 = parseInt(this.value); Game.UpdateMenu();">`
									+ Object.keys(Game.Objects['Temple'].minigame.gods).map((id, index) => 
										`<option value="${index}" ${index == CookieAssistant.config.particular.spirits.slot3 ? 'selected' : ''}>${Game.Objects['Temple'].minigame.godsById[index].name}</option>`
									).join('')
								+ '</select>';
					}
					else
			...
			//建物自動売却
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoSellBuilding', 'CookieAssistant_autoSellBuilding', `${CookieAssistant.texts.autoSellBuildings} ON`, `${CookieAssistant.texts.autoSellBuildings} OFF`, "CookieAssistant.Toggle");
						str += intervalLabel
			...
			str +=	'<div class="listing"><ol style="list-style: inside;list-style-type: decimal;">';
					...
					{
						str += `<li><label>${CookieAssistant.texts.sell} </label>`
			...
			str +=	'</ol>';
			str +=	'<a class="option" ' + Game.clickStr + `="CookieAssistant.addSellConfig(); Game.UpdateMenu(); PlaySound(\'snd/tick.mp3\');">${CookieAssistant.texts.addConfig}</a>`;
			if (CookieAssistant.config.particular.sell.isAfterSell.length > 0)
			{
				str +=	'<a class="option" ' + Game.clickStr + `="CookieAssistant.removeSellConfig(); Game.UpdateMenu(); PlaySound(\'snd/tick.mp3\');">${CookieAssistant.texts.removeLast}</a>`;
			}
			str +=	'</div></div>';
			...
			//ゴールデンスイッチ自動切換え
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoToggleGoldenSwitch', 'CookieAssistant_autoToggleGoldenSwitch', `${CookieAssistant.texts.autoToggle} ` + loc("[Upgrade name 327]Golden switch") + ' ON', `${CookieAssistant.texts.autoToggle} ` + loc("[Upgrade name 327]Golden switch") + ' OFF', "CookieAssistant.Toggle")
				+ '<div class="listing">'
			...
			//ブローカー自動雇用
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoHireBrokers', 'CookieAssistant_autoHireBrokers', `${CookieAssistant.texts.autoHireBrokers} ON`, `${CookieAssistant.texts.autoHireBrokers} OFF`, "CookieAssistant.Toggle");
			if (CookieAssistant.showAllIntervals)
			{
				str += intervalLabel
			...
			//ChocolateEgg
			str +=	'<div class="listing">' + m.ToggleButton(CookieAssistant.config.flags, 'autoChocolateEgg', 'CookieAssistant_autoChocolateEgg', `${CookieAssistant.texts.autoBuy} ` + loc("[Upgrade name 227]Chocolate egg") + ' ON', `${CookieAssistant.texts.autoToggle} ` + loc("[Upgrade name 227]Chocolate egg") + ' OFF', "CookieAssistant.Toggle")
				+ '</div>';
	
			str += "<br />"
			str += m.Header(CookieAssistant.texts.menu_header_3);
			str += '<div class="listing">'
					+ m.ActionButton("CookieAssistant.showAllIntervals = !CookieAssistant.showAllIntervals; Game.UpdateMenu();", (CookieAssistant.showAllIntervals ? `${CookieAssistant.texts.hideIntervalSettings}` : `${CookieAssistant.texts.showAllIntervalSettings}`))
					+ m.ActionButton("CookieAssistant.restoreDefaultConfig(2); CookieAssistant.DoAction(); Game.UpdateMenu();", CookieAssistant.texts.restoreDefault)
					+ m.ActionButton("CookieAssistant.CheckUpdate();", CookieAssistant.texts.checkUpdate)
					+ m.ActionButton("Steam.openLink('https://steamcommunity.com/sharedfiles/filedetails/?id=2596469882');", CookieAssistant.texts.getMoreInfo)
		...
		CookieAssistant.CheckUpdate = async function()
		{
			...
			{
				Game.Notify(CookieAssistant.name, CookieAssistant.texts.notify_5, "", 3)
				return;
			}
	
			Game.Notify(CookieAssistant.name, `<b style="color: #ff8000">${CookieAssistant.texts.notify_6}</b><br><a ${Game.clickStr}="Steam.openLink('${json.assets[0].browser_download_url}')" target="_brank">${CookieAssistant.texts.notify_7}</a>`)
			Game.UpdateMenu();
		}
